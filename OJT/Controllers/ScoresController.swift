...
class ScoresController : UITableViewController {
  ...
  override func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [UITableViewRowAction]? {
    ...
    let add = UITableViewRowAction(style: .Normal, title: "+") { action, index in
        if let contact = self.players?[indexPath.row] as? Contact {
            self.selectedTargetType = "contact"
            self.selectedTarget = contact
            self.performSegueWithIdentifier("ShowScheduleList", sender: self)
        }
    }
    add.backgroundColor = UIColor.greenTintColor()
    
    // Remove
    let remove = UITableViewRowAction(style: .Normal, title: "-") { action, index in
  ...
  override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
    ...
    if segue.identifier == "ShowScheduleList" {
        let scheduleController = segue.destinationViewController as! ScoreScheduleController
        scheduleController.selectedTarget = selectedTarget
